...
import { translateJSON as coreTranslateJSON } from '@shipi18n/core'

/**
 * Vite plugin for automatic i18n translation at build time — bring your own LLM.
 *
 * Uses YOUR OpenAI/Anthropic key via @shipi18n/core. No Shipi18n account or hosted API.
 *
 * @param {Object} options - Plugin configuration
 * @param {'anthropic'|'openai'|Object} options.provider - LLM provider or a custom { complete } adapter (default: 'anthropic')
 * @param {string} options.apiKey - LLM API key (optional; falls back to ANTHROPIC_API_KEY / OPENAI_API_KEY)
 * @param {string} options.model - Override the provider's default model (optional)
...
export default function shipi18nPlugin(options = {}) {
  ...
  // Validation — no Shipi18n key required; the LLM key is resolved at translate time
  // (from `apiKey` or the provider's env var), so a missing key surfaces as a build-time
  // translation error and is handled by the fallback path below.
  ...
  return {
    ...
    async buildStart() {
      ...
      console.log(`   Provider: ${typeof provider === 'string' ? provider : 'custom'}`)
      ...
      for (const fileName of sourceFiles) {
        ...
        if (!translations) {
          ...
          try {
            translations = await translateAllLanguages({
              provider,
              apiKey,
              model,
              json: sourceJson,
...
/**
 * Translate one source JSON object into every target language via @shipi18n/core.
 * Returns a { [langCode]: translatedJson } map — the same shape the fallback logic expects.
 */
async function translateAllLanguages({ provider, apiKey, model, json, sourceLanguage, targetLanguages }) {
  const out = {}
  for (const to of targetLanguages) {
    const { result } = await coreTranslateJSON({
      content: json,
      from: sourceLanguage,
      to,
      provider,
